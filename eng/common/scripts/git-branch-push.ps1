...
param(
    ...
    [string] $ForkName = "azure-sdk-fork",
...
Write-Host "git remote add $ForkName $GitUrl"
git remote add $ForkName $GitUrl
...
{
    $needsRetry = $false
    Write-Host "git push $ForkName $PRBranchName $PushArgs"
    git push $ForkName $PRBranchName $PushArgs
    $tryNumber++
    if ($LASTEXITCODE -ne 0)
    {
        $needsRetry = $true
        ...
        try
        {
            ...
            Write-Host "git reset --hard $ForkName/${PRBranchName}"
            git reset --hard $ForkName/${PRBranchName}
            if ($LASTEXITCODE -ne 0)
            {
